...
class Simulator(object):
    ...
    def __init__(self, env, size=None, update_delay=1.0, display=True):
        ...
        self.size = size if size is not None \
            else ((self.env.grid_size[0] + 1) * self.env.block_size, (self.env.grid_size[1] + 1) * self.env.block_size)
        ...
        for agent, state in self.env.agent_states.iteritems():
            ...
            if hasattr(agent, '_sprite') and agent._sprite is not None:
                ...
                rotated_sprite = agent._sprite if state['heading'] == (1, 0) else \
                    self.pygame.transform.rotate(agent._sprite,
                                                 180 if state['heading'][0] == -1 else state['heading'][1]*-90)
                self.screen.blit(
                    rotated_sprite,
                    self.pygame.rect.Rect(
                        agent_pos[0]-agent._sprite_size[0]/2, agent_pos[1]-agent._sprite_size[1]/2,
